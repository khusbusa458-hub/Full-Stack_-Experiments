...
const PostComposer = ({ onPostCreate, onError }) => {
  ...
  const handlePlatformChange = (e) => {
    ...
    const limit = WORD_LIMITS[newPlatform];
    const words = countWords(content);
    if (words > limit) {
      onError(`Word limit reached for ${newPlatform}! Maximum allowed is ${limit} words.`);
      const truncatedText = content.trim().split(/\s+/).slice(0, limit).join(' ');
  ...
  const handleSubmit = (e) => {
    ...
    if (!trimmed) return;
    onPostCreate({ platform, content: trimmed });
    setContent('');
    onError(null);
  };
